...
void CALIBRATOR::init(){
    ...
    if(strcmp(camera_name.c_str(), "kinect_v2") == 0){
        ...
        CAMERA_kinect_v2::Ptr camera;
        camera->init(_global_parameters);
        _camera = camera;
    ...
    _global_parameters.get_point_in_camera_frame().resize(_global_parameters.get_number_of_points(), 4);
    _global_parameters.get_point_in_robot_frame().resize(_global_parameters.get_number_of_points(), 4);
...
void CAMERA_kinect_v2::init(Data_config& parameters){
    ...
    parameters.get_camera_character().readFromXMLFile(camera_file_path);
    _camera_spinner.reset(new ros::AsyncSpinner(1));
    _camera_spinner->start();
}

void CAMERA_kinect_freenect::init(Data_config &parameters){
    ROS_INFO("CAMERA_kinect_freenect: initializing ......");
    _syncronized_freenect_sub.reset(new rgbd_utils::RGBD_Subscriber("/camera/rgb/camera_info",
    ...
    ROS_INFO("CALIBRITOR: initializing kinect freenect camera finished!");
    _camera_spinner.reset(new ros::AsyncSpinner(1));
    _camera_spinner->start();
    ROS_INFO("CAMERA_kinect_freenect: initializing finished.");
}

void CAMERA_kinect_openni::init(Data_config &parameters){
    _syncronized_camera_sub.reset(new rgbd_utils::RGBD_Subscriber("/camera/rgb/camera_info",
                                                                  "/camera/rgb/image_raw",
                                                                  "/camera/depth/camera_info",
                                                                  "/camera/depth_registered/image_raw",
                                                                  _nh_camera));
    _camera_topics_sub = _nh_camera.subscribe<sensor_msgs::Image>("/camera/depth/image_raw", 1, &CAMERA_kinect_openni::camera_topics_start_publishing_cb, this);
    std::string camera_file_path;
    _nh_camera.getParam("/camera_file_path", camera_file_path);
    parameters.get_camera_character().readFromXMLFile(camera_file_path);
    _camera_spinner.reset(new ros::AsyncSpinner(1));
...
void BAXTER::init(){
    ...
    if(strcmp(baxter_arm.c_str(), "left") == 0)
        _eef_state_sub.reset(new ros::Subscriber(_nh_robot.subscribe("/robot/limb/left/endpoint_state", 10, &BAXTER::eef_Callback, this)));
    else if(strcmp(baxter_arm.c_str(), "right") == 0)
        _eef_state_sub.reset(new ros::Subscriber(_nh_robot.subscribe("/robot/limb/right/endpoint_state", 10, &BAXTER::eef_Callback, this)));
    _robot_spinner.reset(new ros::AsyncSpinner(1));
    _robot_spinner->start();
    ROS_INFO("BAXTER: initializing finished.");
}

void CRUSTCRAWLER::init(){
    _eef_state_sub.reset(new ros::Subscriber(_nh_robot.subscribe("/crustcrawler/endpoint_state", 10, &CRUSTCRAWLER::eef_Callback, this)));
    _robot_spinner.reset(new ros::AsyncSpinner(1));
    _robot_spinner->start();
}
